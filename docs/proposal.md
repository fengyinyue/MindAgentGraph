...
5. 用户审查和调整节点。
6. 对关键节点执行细化，生成模块清单、状态设计、交互设计、文件改动范围和验收标准。
7. AI 按 DAG 顺序执行非 Code 节点，先沉淀上游输出。
8. Code 节点在继承上游输出后，再调用代码执行工具实现或修改工程。
9. 执行输出回写到节点和 Memory。

规划应分为“粗规划”和“细化规划”两层：

- 粗规划负责把一句话目标拆成高层节点图，例如需求、模块设计、UI 设计、数据模型、实现计划和 Code 节点。
- 细化规划负责把关键节点展开成可执行细节，例如需要哪些模块、每个模块的职责、状态流转、文件范围、接口约定和验收条件。
- Code 节点不应该只依赖用户的一句话目标，而应该继承上游细化节点的输出后再执行。
- 如果上游节点尚未执行出 `output`，下游节点只能依据自身 `title`、`purpose`、Prompt 和 Memory 推断，结果不稳定。
- 产品应提供“深化节点 / Expand Details”能力：用户可以选中一个粗粒度节点，让 AI 基于当前节点和上下游关系生成更细的子节点、子图或详细任务清单。
